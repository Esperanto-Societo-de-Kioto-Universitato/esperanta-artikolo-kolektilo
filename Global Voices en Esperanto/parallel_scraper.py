...
# -*- coding: utf-8 -*-
"""
Global Voices en Esperanto の記事収集を複数プロセスで分割実行し、最後に結合して書き出す CLI。
...
DEFAULT_BASE_URL = "https://eo.globalvoices.org"
SOURCE_LABEL = "Global Voices en Esperanto (eo.globalvoices.org)"
PREFIX = "global_voices_eo"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Global Voices en Esperanto 期間指定スクレイパー (並列版)")
...
def _report_worker_failure(worker: WorkerArgs, exc: Exception) -> None:
    print(
...
def _report_failures(
    failed_chunks: List[Tuple[WorkerArgs, Exception]], article_failures: List[str]
) -> None:
    if not failed_chunks and not article_failures:
        return
    print(
        f"[WARN] 失敗: 期間 {len(failed_chunks)} 件 / 記事 {len(article_failures)} 件（詳細は標準エラー出力）"
    )
    if failed_chunks:
        print(
        ...
        for worker, exc in sorted(failed_chunks, key=lambda item: item[0].index):
            print(
                f"  - --start {worker.cfg.start_date} --end {worker.cfg.end_date}  ({type(exc).__name__}: {exc})",
                file=sys.stderr,
            )
    if article_failures:
        ...
        with ProcessPoolExecutor(max_workers=actual_workers) as executor:
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    _report_worker_failure(worker, exc)
                    failed_chunks.append((worker, exc))
